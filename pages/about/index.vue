<template>
  <div class="w-full rounded shadow-sm bg-white p-20">
    <div>
      Component: <code class="inline-block rounded p-1 bg-gray-200">about/index.vue</code>
    </div>
    <div class="mt-8">
    ...
    <div class="-mx-4 mt-12 flex items-center">
      ...
      <nuxt-link class="rounded bg-blue-500 text-white hover:bg-blue-400 mx-4 p-4" to="/about/me">
        Go to about/me
      </nuxt-link>
    </div>
  </div>
...
<script>
  ...
  export default {
    transition:{
      enter (el, done) {
        console.warn('entering about/index.vue page')
      ...
      leave (el, done) {
        console.warn('leaving about/index.vue page')
        gsap.timeline({ onComplete: done, defaults: { ease: 'back.inOut(3)', duration: 0.5 } })
          .to(el, {
            position: 'absolute',
            y: -20,
